...
function setupExternalFunding() {
  const source = $("depositFundingSource");
  const fields = $("externalFundingFields");
  const input = $("externalFundingAccount");
  const label = $("externalFundingLabel");
  ...
  source.addEventListener("change", () => {
    ...
    if (label) {
      if (value === "bank") {
        ...
        label.textContent = "Payment Account";
        input.placeholder = "Use secure payment checkout";
      } else {
        label.textContent = "Wallet";
        input.placeholder = "Enter wallet/account identifier";
      }
    }
  });
}
...
/* =========================================================
   VICKY PAY — CONNECTED ACCOUNT DEPOSIT FIX
   ========================================================= */

async function loadDepositLinkedAccounts() {
  const select = document.getElementById("depositLinkedAccount");
  const status = document.getElementById("depositLinkedAccountStatus");
  ...
  try {
    ...
    if (!token) {
      select.innerHTML =
        '<option value="">Please sign in first</option>';

      if (status) {
        status.textContent =
          "Sign in to use a connected funding account.";
      }
    ...
    const connected = accounts.filter(
      account =>
        String(account.status || "connected").toLowerCase() ===
        "connected"
    );
    ...
    if (!connected.length) {
      ...
      if (status) {
        status.textContent =
          "Connect a bank account before making a funded deposit.";
      }
    ...
    if (status) {
      status.textContent =
        `${connected.length} connected account${connected.length === 1 ? "" : "s"} available.`;
    }

  } catch (error) {
    console.error(
      "Failed to load deposit linked accounts:",
      error
    );
...
function getSelectedDepositLinkedAccountId() {
  const select =
    document.getElementById("depositLinkedAccount");
...
document.addEventListener(
  "DOMContentLoaded",
  () => {
    loadDepositLinkedAccounts();
  }
);

/* =========================================================
   END CONNECTED ACCOUNT DEPOSIT FIX
   ========================================================= */



/* =========================================================
   VICKY PAY — CONNECTED FUNDING ACCOUNT SYSTEM
   ========================================================= */

async function loadVickyPayLinkedAccounts() {
  const selects = [
    document.getElementById("depositLinkedAccount"),
    document.getElementById("screenDepositLinkedAccount")
  ].filter(Boolean);
  ...
  try {
    ...
    const statuses = [
      document.getElementById("depositLinkedAccountStatus"),
      document.getElementById("screenDepositLinkedAccountStatus")
    ].filter(Boolean);

    statuses.forEach(status => {
      status.textContent = connected.length
        ? `${connected.length} connected account${connected.length === 1 ? "" : "s"} available.`
        : "No connected funding account is available.";
    });
    ...
    statuses.forEach(status => {
      status.textContent =
        error.message || "Unable to load connected accounts.";
    });
  }
}

function getVickyPayDepositAccountId(screen = false) {
  const id = screen
    ? document.getElementById("screenDepositLinkedAccount")?.value
    : document.getElementById("depositLinkedAccount")?.value;
...
document.addEventListener("DOMContentLoaded", () => {
  setupExternalFunding();
});

/* Vicky Pay external deposit funding selector */
(function setupVickyPayDeposit() {
  function init() {
    const source = document.getElementById("depositFundingSource");
    if (!source) return;

    const bank = document.getElementById("depositBankFields");
    const card = document.getElementById("depositCardFields");
    const wallet = document.getElementById("depositWalletFields");
...
/* FORCE VICKY PAY DEPOSIT SOURCE SELECTOR */
...
/* VICKY PAY - FORCE DEPOSIT UI */
window.openVickyPayDeposit = function () {
  const screen = document.getElementById("depositScreen");
  if (!screen) return;
  ...
  function updateFundingFields() {
    ...
    if (source.value === "bank") {
      bank?.classList.remove("hidden");
    } else if (source.value === "card") {
      card?.classList.remove("hidden");
    } else if (source.value === "wallet") {
      wallet?.classList.remove("hidden");
    }
  }

  source.onchange = updateFundingFields;
  updateFundingFields();
};


/* FINAL DEPOSIT SCREEN INITIALIZER */
document.addEventListener("click", function () {
  setTimeout(function () {
    ...
    if (
      ...
      if (typeof window.openVickyPayDeposit === "function") {
        window.openVickyPayDeposit();
      }
    }
  }, 50);
});
